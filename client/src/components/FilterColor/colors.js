...
const findColors = name => {
	...
	if (color) {
		...
		const color = names.find(item => item[1].split(' ')[0] === name);
		if (color) {
			return color[0];
		} else {
			const color = names.find(item => item[1].split(' ')[1] === name);
			if (color) {
				return color[0];
			} else {
...
export const deleteAllRepited = array => {
	let unifie = [];
	array.forEach(elem => {
		elem.names.forEach(elem2 => {
			unifie.push(elem2);
		});
	});
	let set = new Set(unifie.map(JSON.stringify));
	let arrSinDuplicaciones = Array.from(set).map(JSON.parse);
	let clean = arrSinDuplicaciones.filter(elem => {
...
export const filter = (array, value) => {
	...
	if (value !== '') {
		for (let i = 0; i < array.length; i++) {
			for (let f = 0; f < array[i].names.length; f++) {
				//console.log(array[i].names);
				//for (let j = 0; j < typeFilters.length; j++) { //esto es por si queremos comparar con varios colores
				if (array[i].names[f].includes(value)) {
					arre.push(array[i]);
					//}
				}
			}
...
export const onlyThreeColorGrid = (data, silhoutte, id) => {
	let additionalItems = data.filter(item => item.silhoutte === silhoutte && item.id !== id);
	let allIdem = data.filter(item => item.silhoutte === silhoutte && item.id === id).concat(additionalItems),
		all = {},
	...
	if (!all.colorNameThumbnailAnd_id.length) return null;
	return all;
};
